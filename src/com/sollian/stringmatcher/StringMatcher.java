...
public class StringMatcher {
	 ...
	 *
	 * @param pinyins
	 *            如：单独-{"dan du", "shan du", "chan du"} 不区分大小写
	 * @param target
	 *            如：dandu，不区分大小写
	 * @param fullMatch
	 *            true：target子串匹配整个pinyin或者pinyin首字母时匹配成功；
	 *            false：匹配pinyin第0个字符开始的任意大于0的字符时匹配成功
	 * @param enableSampledMatch
	 *            是否开启非连续的匹配。如lx匹配lishouxian，true匹配成功，false匹配失败
	 */
	 ...
	 *            </ul>
	 * @param forceMatchFlag
	 *            true执行精确颗粒度（flag）的匹配；false执行最大颗粒度的匹配
	 * @param inOrder
	 *            是否按照目标字符串中单词（flag为{@link #FLAG_MATCH_WORD}时）或者字符（flag为
	 *            {@link #FLAG_MATCH_CHAR}时）的顺序进行匹配。flag为
	 *            {@link #FLAG_MATCH_SENTENCE}时无效
	 * @param customSplit
	 *            flag为{@link #FLAG_MATCH_WORD}
	 *            时，true使用splitChars作为单词分隔符；false使用默认方法分割单词
	 * @param splitChars
	 *            flag为{@link #FLAG_MATCH_WORD}时，作为单词间的分隔符。若为null，则使用
